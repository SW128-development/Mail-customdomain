...
export async function initializeCloudflareProvider() {
  ...
  try {
    ...
    if (autoInit === 'true') {
      console.log('[Cloudflare] Auto-init already completed')
      return
    }
    ...
    if (!data.success || !data.workerInfo) {
      console.log('[Cloudflare] No existing configuration found')
      return
    }
    ...
    if (existingProvider) {
      console.log('[Cloudflare] Provider already exists:', existingProvider.name)
    ...
    console.log('[Cloudflare] Auto-added provider:', newProvider.name)
    console.log('[Cloudflare] Domains available:', workerInfo.domains.join(', '))
    
    // Trigger a page reload to ensure the provider is loaded
    if (window.location.pathname === '/') {
      window.location.reload()
    }
    
  } catch (error) {
    console.error('[Cloudflare] Auto-init error:', error)
  }
}
